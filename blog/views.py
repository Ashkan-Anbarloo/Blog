...
from .forms import TicketForm , CommentForm , PostForm , SearchForm , UserRegisterForm , UserEditForm , AccountEditForm
...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            # تعریف شباهت‌ها برای هر فیلد
            post_title_sim = TrigramSimilarity('title', query)
            post_desc_sim = TrigramSimilarity('description', query)
            
            # برای تصاویر، چون چندتا هستند از Max استفاده می‌کنیم (بهترین شباهت بین تمام عکس‌های پست)
            img_title_sim = Max(TrigramSimilarity('images__title', query))
            img_desc_sim = Max(TrigramSimilarity('images__description', query))
            
            # ترکیب همه شباهت‌ها و انتخاب بزرگترین (Greatest)
...
def page_account(request):
    ...
    return render(request , 'blog/page_account.html' , context)